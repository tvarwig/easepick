...
export class Core {
  ...
  public show(event?): void {
    ...
    let inputElement = document.querySelector("#daterangepickertext");

    // Check if the sibling exists and has the class 'rightside'
    if (inputElement && inputElement.classList.contains("rightside")) {
      this.ui.container.style.left = "-588px";
    } else {
